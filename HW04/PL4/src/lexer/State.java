		...
		public TransitionOutput transit(ScanContext context) {
			...
			switch ( ch.type() ) { // 각자 type관하여 처리
				...
				case SPECIAL_CHAR: //special charactor가 들어온 경우 
					if ( v == '+' || v == '-' ) { //부호인 경우 상태 반환
						context.append(v); // context에 추가
						return GOTO_SIGN; // GOTO_SIGN으로 이동		
					}
					else if ( v == '#' ) {  //boolean경우 상태 반환
						context.append(v); // context에 추가
						return GOTO_SHARP;	// GOTO_SHARP으로 이동	
					}
					else { 
						context.append(v); // context에 추가
						return GOTO_MATCHED(TokenType.fromSpecialCharactor(v),context.getLexime());//이외 type을 알아내서 상태 반환
